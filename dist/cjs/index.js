...
function ChatwootScript(props) {
    var token = props.token;
    var BASE_URL = "https://app.chatwoot.com";
    var SCRIPT_URL = BASE_URL + "/packs/js/sdk.js";
    //const status = useScript(SCRIPT_URL)
    //console.log("status", status)
    function onLoadHandler(e) {
        e.target.setAttribute("data-status", "ready");
        if (!token) {
    ...
    function onErrorHandler(e) {
        e.target.setAttribute("data-status", "error");
    }
    var checkExistingScript = function () { return document.querySelector("script[src=\"".concat(SCRIPT_URL, "\"]")); };
    react.exports.useEffect(function () {
        if (typeof token !== "string") {
            console.error("Chatwoot SDK requires token.");
        }
        // Check if it is attached to DOM before
        var existingScriptEl = checkExistingScript();
        if (!existingScriptEl) {
            var scriptEl_1 = document.createElement("script");
            scriptEl_1.id = "chatwoot-script";
            scriptEl_1.src = SCRIPT_URL;
            scriptEl_1.async = true;
            scriptEl_1.defer = true;
            scriptEl_1.addEventListener("load", onLoadHandler);
            scriptEl_1.addEventListener("error", onErrorHandler);
            document.body.appendChild(scriptEl_1);
            return function () { return scriptEl_1.removeEventListener("load", onLoadHandler); };
        }
    }, []);
    return (React.createElement(react.exports.Fragment, null));
}
...
var _react$1 = _interopRequireDefault(react.exports);
...
Object.defineProperty(headManager, "__esModule", {
    value: true
});
headManager.isEqualNode = isEqualNode;
headManager.default = initHeadManager;
headManager.DOMAttributeNames = void 0;
...
headManager.DOMAttributeNames = DOMAttributeNames;
...
var _default = Script;
script$1.default = _default;

var script = script$1;

function ChatwootNextScript(props) {
    var token = props.token, restProps = __rest(props, ["token"]);
    var BASE_URL = 'https://app.chatwoot.com';
    var SCRIPT_URL = BASE_URL + '/packs/js/sdk.js';
    ...
    return React.createElement(script, __assign({ id: "chatwood-script", src: SCRIPT_URL, onLoad: onLoadHandler }, restProps));
}

exports.ChatwootNextScript = ChatwootNextScript;
exports.ChatwootScript = ChatwootScript;
